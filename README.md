...
- **`?data=api` 를 붙이면 실제 API(`api.loresentry.com`)를 쓴다.** 로그인·계정·프로젝트·파일·문서·버전·검색·메모·즐겨찾기·작업공간 상태가 서버에서 오고, 그래프·최신화·AI 챗·가이드만 mock 이 채운다. 인증은 쿠키와 `X-LS-CSRF` 를 쓰므로 실제 Google 로그인이 필요하다. 포트 단위 전환과 아직 안 되는 기능은 [frontend/api-adapter.md](https://github.com/soma-lorekeeper/docs/blob/main/frontend/api-adapter.md) 에 있다.
