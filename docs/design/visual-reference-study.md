...
| `20 · Visual Direction · Recommended Blend · APPROVAL CANDIDATE` (`K4irXB`) | 위 세 공식 참고자료와 2026-08-28 사용자 선택 조합 | 17의 차분한 표면, 18의 원고 대비·1.78 줄 간격, 19보다 약한 패널 경계(`#403D37`)와 선택 상태(`#332F28`, `#D2C4A6`)를 결합한다. 아이콘은 중간 굵기 400을 사용한다. |
...
- 선택: `20 · Visual Direction · Recommended Blend · APPROVAL CANDIDATE`
...
`LOREKEEPER-324`에서는 선택 화면을 다시 꾸미지 않고 사용자가 남긴 피드백을
조정 기준으로 대조했다.

- 유지 요청: 권장 조합 전체와 기존 C의 정보 구조를 그대로 유지했다.
- 수정 요청: 사용자가 별도로 지정한 항목이 없어 임의 조정을 추가하지 않았다.
- 제외 요청: 사용자가 별도로 지정한 항목이 없어 기존 선택안에서 제거하지 않았다.
- 승인 전 상태: 프레임 이름을 `APPROVAL CANDIDATE`로 명시해 선택과 최종 승인을
  구분했다.

후보 화면은 구조 기준본과 동일한 145개 노드·문구·선택 상태를 유지한다. Pencil
검사에서 새 레이아웃 잘림은 발견되지 않았으며, 그라디언트·발광·배경 이미지나
추가 장식도 도입하지 않았다.
