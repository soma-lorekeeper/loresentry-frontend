# Workflow

페이지와 상태 사이의 이동을 기록한다. 각 Workflow는 다음 형식을 사용한다.

```text
출발 화면 → 사용자 행동 또는 조건 → 도착 화면
```

- [서비스 진입](./service-entry.md)
- [Google OAuth 인증](./authentication.md)
- [프로젝트 진입](./project-entry.md)
- [프로젝트 생성](./project-creation.md)
- [프로젝트 휴지통](./project-trash.md)
- [프로젝트 전환](./project-switching.md)
- [즐겨찾기](./favorites.md)
- [파일·폴더 이동](./item-movement.md)
- [파일·폴더 생성과 이름 변경](./item-creation-and-renaming.md)
- [사용자 생성 섹션 삭제](./section-deletion.md)
- [파일·폴더 휴지통](./workspace-trash.md)
- [작업공간 콘텐츠 열기](./workspace-navigation.md)
- [작업공간 검색](./workspace-search.md)
- [작업공간 그래프](./workspace-graph.md)
- [메모 탭](./project-memos.md)
- [메모 패널 배치](./memo-panel-layout.md)
- [속성 문서 편집](./structured-file-editing.md)
- [파일 헤더 동작](./file-header-actions.md)
- [이벤트 시간 흐름 편집](./event-timeline-editing.md)
- [작업공간 설정과 도움말](./workspace-settings-and-help.md)
